...
using XML,Sockets,JSON3,StructTypes,Observables,OrderedCollections
...
function fill_ids()
    ...
    for p_i in param_details
        ...
        name_node = find_tag(p_i,"Name")
        for nm in name_node
            has_single_child(nm) || continue
            if nodetype(nm.children[])==XML.Text
                D[current_key] = nm.children[].value
            end
        end 
        #@show current_key
    end
end
function has_single_child(node)
    return !isnothing(node.children) && length(node.children) == 1
end
